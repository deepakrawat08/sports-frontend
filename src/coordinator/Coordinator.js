...
import {
	teamByStatusAndCoord,
	individualByStatus,
	updateIStatus,
	updateTStatus,
	individualByStatusAndCoord,
} from "../Admin/helper/getTeam";
import {
	getBranchName,
	getGameName,
	getYear,
...
export default function Coordinator() {
	...
	const setStatus = (value) => {
		...
		if (currentReg !== value ) {
			if ( ["VB", "CR", "FB"].includes(coordinatorOf)) {
				setModeTab(team)
			} else {
	...
	const coordinatorOf = getCoordinatorOf(user.role);

	const getAllTeams = () => {
		console.log(currentReg);
		teamByStatusAndCoord(user._id, token,currentReg, coordinatorOf).then((data) => {
			if (!data.fetchError) {
				if (data.error) {
					setteamDB({
						...teamDB,
	...
	const getAllIndividual = () => {
		individualByStatusAndCoord(user._id, token, currentReg,coordinatorOf).then((data) => {
			if (!data.fetchError) {
				if (data.error) {
					setindividualDB({
						...individualDB,
	...
	const regNav = () => {
		return (
			<div id="reg_tab">
				...
				>
					<span className="reg_section">Pending Registration</span>
				</button>
				<button
					onClick={() => {
						setStatus(accepted);
				...
				>
					<span className="reg_section">Accepted Registration</span>
				</button>
				<button
					onClick={() => {
						setStatus(rejected);
				...
				>
					<span className="reg_section">Rejected Registration</span>
				</button>
			</div>
		);
	};
			...
			return (
				<div id="adupdate_status_section">
					<button
						...
						id="adreject_reg"
						onClick={() => {
							updateTeamStatus(teamCode, rejected,user.rollNo);
						}}
					>
						Reject
					</button>
					<button
						...
						id="adaccept_reg"
						onClick={() => {
							updateTeamStatus(teamCode, accepted,user.rollNo);
						}}
					>
						Accept
					</button>
			...
			return (
				<div id="adupdate_status_section">
					<button
						...
						id="adreject_reg"
						onClick={() => {
							updateIndividualStatus(regNo, rejected,user.rollNo);
						}}
					>
						Reject
					</button>
					<button
						...
						id="adaccept_reg"
						onClick={() => {
							updateIndividualStatus(regNo, accepted,user.rollNo);
						}}
					>
						Accept
					</button>
	...
	return (
		<Base title="Coordinator" description="This is coordinator dashboard">
			{regNav()}
			{!error &&
				!fetchError &&
				!loading &&
